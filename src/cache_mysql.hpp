...
template <typename MySqlCachePolicy>
storages::postgres::Query MySqlCache<MySqlCachePolicy>::GetAllQuery() {
...
template <typename MySqlCachePolicy>
storages::postgres::Query MySqlCache<MySqlCachePolicy>::GetDeltaQuery() {
...
void MySqlCache<MySqlCachePolicy>::Update(
  ...
  size_t changes = 0;
  // Iterate clusters
  for (std::shared_ptr<storages::mysql::Cluster> cluster : clusters_) {
    if (chunk_size_ > 0) {
      ...
      auto resultValues = cluster->Execute(userver::storages::mysql::ClusterHostType::kPrimary, query.Statement()).AsVector<ValueType>();
